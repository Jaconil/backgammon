...
void FreeAIStruct(S_AIFunctions* aiFunctions)
{
    if (aiFunctions->ai[0] != NULL)
        SDL_UnloadObject(aiFunctions->ai[0]);

    if (aiFunctions->ai[1] != NULL)
        SDL_UnloadObject(aiFunctions->ai[1]);
}
...
int LoadAPI(char* path, S_AIFunctions* aiFunctions, int index)
{
    ...
    if ((aiFunctions->AI_InitLibrary[index] = (pfInitLibrary)SDL_LoadFunction(aiFunctions->ai[index], "InitLibrary")) == NULL) return 0;
    if ((aiFunctions->AI_StartMatch[index] = (pfStartMatch)SDL_LoadFunction(aiFunctions->ai[index], "StartMatch")) == NULL) return 0;
    if ((aiFunctions->AI_StartGame[index] = (pfStartGame)SDL_LoadFunction(aiFunctions->ai[index], "StartGame")) == NULL) return 0;
    if ((aiFunctions->AI_EndGame[index] = (pfEndGame)SDL_LoadFunction(aiFunctions->ai[index], "EndGame")) == NULL) return 0;
    if ((aiFunctions->AI_EndMatch[index] = (pfEndMatch)SDL_LoadFunction(aiFunctions->ai[index], "EndMatch")) == NULL) return 0;
    if ((aiFunctions->AI_DoubleStack[index] = (pfDoubleStack)SDL_LoadFunction(aiFunctions->ai[index], "DoubleStack")) == NULL) return 0;
    if ((aiFunctions->AI_TakeDouble[index] = (pfTakeDouble)SDL_LoadFunction(aiFunctions->ai[index], "TakeDouble")) == NULL) return 0;
    if ((aiFunctions->AI_MakeDecision[index] = (pfMakeDecision)SDL_LoadFunction(aiFunctions->ai[index], "MakeDecision")) == NULL) return 0;
